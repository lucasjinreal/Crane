...
- **`2026.07.03`**: 🗜️ Qwen 3.5 quantization — load community **GGUF** files directly (`--model-path model.gguf`, llama.cpp `qwen35` layout incl. the hybrid GDN blocks, arch auto-detected from the header; put `tokenizer.json` next to the file), or quantize a safetensors checkpoint at load time with **`--quant q4k|q8_0|…`** / `CRANE_ISQ` (in-situ quantization via candle `QMatMul`, no conversion step). Qwen3.5-0.8B serves in ~1.2 GB from a Q4_0 GGUF vs ~3.7 GB full precision on Apple Silicon.
